...
def test_item_filter():
    ...
    # Should handle when y=None but X is not
    item_filter = ItemFilter(lambda df: df["b"] == "c")
    xo, yo = item_filter.run(X, None)
    assert yo is None
    assert isinstance(xo, pd.DataFrame)
    assert xo.shape[0] == 2
    assert xo.shape[1] == 3
    ...
    # Should return None if no rows match condition
    item_filter = ItemFilter(lambda df: df["b"] == "d")
    node_output = item_filter.run(X, y)
    assert is_empty(node_output)
